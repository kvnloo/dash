...
| | |
|---|---|
| **This repo** | https://github.com/kvnloo/dash |
| Clone | `git clone https://github.com/kvnloo/dash.git` |
...
1. Clone or open https://github.com/kvnloo/dash. `cd` into that tree. `git fetch origin`. `python3 .verified-oss-loop/rollout.py show`. Branch from `origin/$(python3 .verified-oss-loop/rollout.py get worker_base)` unless the issue names another base. Day-pass PRs target `feature_target`. Overnight PRs target `overnight_target`.
...
6. Fail, then pass (`skills/tdd/SKILL.md` or `.cursor/skills/tdd/SKILL.md`). Unit: `bun test app/src bridge/src shared`. Mutation: `bun scripts/mutate.ts` (score ≥ 80). Do not run Stryker. Runtime: `.cursor/skills/verify-dash`.
...
8. Open a PR on https://github.com/kvnloo/dash at `feature_target` (day) or `overnight_target` (overnight) with `.github/PULL_REQUEST_TEMPLATE.md` filled. Bind evidence to `head_revision`. **Do not merge `main` or `dev`.**
