...
- Orchestra pane lists the pinned AODL network in `app/src/catalog/` (`aodl-catalog.json` + `encodings/visual.json` / `ir-map.json`). `HarnessAvatar` draws `VisualCore` from `ownerRules` (unknown harness ids fail closed to the declared unknown look). Unknown ids and not-inferred silhouettes fail closed. `o8` and `firstmate` are not Orchestra graphs. No letter-in-circle, no OpenAvatar marks.
